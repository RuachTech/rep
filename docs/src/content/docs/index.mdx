---
title: REP — Runtime Environment Protocol | Secure Runtime Env Vars for Frontend Apps
description: Inject environment variables into browser apps at container runtime — not build time. AES-256 encryption, secret detection, hot reload. Build once, deploy anywhere.
template: splash
hero:
...
REP operates at the **infrastructure layer** in production — no build-tool plugin required, no framework coupling. It works with any frontend stack that produces HTML. Optional dev-time plugins exist purely for local DX (see below).
...
<CardGrid>
  ...
  <Card title="Local Development" icon="terminal">
    Install the [Vite](/reference/plugins/vite/) or [Next.js](/reference/plugins/next/) plugin for zero-config `dev` support, or the CLI wraps the gateway to proxy any dev server with hot reload.
  </Card>
...
| **REP** | **3-tier** | **AES-256-GCM** | **HMAC + SRI** | **SSE** | **~7MB Go** |
...
<CardGrid>
  <Card title="Gateway" icon="rocket">
    Go binary (~7MB). Zero dependencies. `FROM scratch` compatible. Proxy or embedded mode.
  </Card>
  ...
  <Card title="Build-Tool Plugins" icon="setting">
    Vite (`repPlugin()`) and Next.js (`RepScript`) plugins inject the same payload during `npm run dev` — no gateway needed locally.
  </Card>
  <Card title="Codemod" icon="magnifier">
    `npx @rep-protocol/codemod` automates migration from Vite, CRA, or Next.js `process.env`/`import.meta.env` reads to `rep.get()`.
  </Card>
</CardGrid>
