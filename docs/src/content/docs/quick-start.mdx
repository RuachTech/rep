---
title: Quick Start — Runtime Environment Variables in 5 Minutes
description: Get runtime environment variables in your frontend app in under 5 minutes. Install the SDK, rename your env vars, and run the gateway. No manifest required.
---
...
REP works the same way with a single `index.html` and no bundler — see the [Simple HTML example](/examples/simple-html/) for the SDK loaded straight from a CDN.

## Build-tool plugins for local dev

Running `vite dev` or `next dev` without the gateway? Install the [Vite](/reference/plugins/vite/) or [Next.js](/reference/plugins/next/) plugin to get the same injected payload during development — no second terminal, no gateway binary needed locally.
