...
<Tabs>
  ...
  That's it — `rep.get()` and `rep.getSecure()` now work in `vite dev`. No gateway needed.
  </TabItem>
  ...
  `RepScript` renders the REP payload during `next dev` and returns `null` in production — the gateway handles injection in prod.
  </TabItem>
</Tabs>
